...
const helpUnsubscribe = "HOW TO SUBSCRIBE?\
...
const helpCommands = {
    ...
    '/snipe': {
        'add [name] [[parts]]': 'Add new snipe with provided ninneko parts information (Up to 3)',
...
const commmandsToString = (input) => {
    const commands = [];
    for (const command of Object.keys(input)) {
        if (typeof input[command] !== 'string') {
            for (const subCommand of Object.keys(input[command])) {
                const description = input[command][subCommand];
                commands.push(`${command} ${subCommand} ${description ? '<- ' + description : ''}`);
            }
        }
        else {
            const description = input[command];
            commands.push(`${command} ${description ? '<- ' + description : ''}`);
        }
    }
    return commands.join('\n\n');
};
const helpSubscribed = 'In construction!!!\n\n' + commmandsToString(helpCommands);
exports.default = { welcome, helpUnsubscribe, helpSubscribed };
