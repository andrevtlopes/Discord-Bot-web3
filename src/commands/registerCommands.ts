...
const commands = [
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName('id_transacao')
                .setDescription(
                    'O hash da transação feita'
                )
                .setRequired(true)
        )
        .toJSON(),
    new SlashCommandBuilder()
        .setName('linkarwallet')
        .setDescription('Linkar Wallet')
        .addStringOption((option) =>
            option
                .setName('endereço_bep20')
                .setDescription(
                    'Endereço da carteira do usuário (somente um por conta pode ser usado)'
                )
                .setRequired(true)
        )
        .toJSON(),
    new SlashCommandBuilder()
        .setName('vertempo')
        .setDescription('Ver tempo restante')
        .toJSON(),
    // new SlashCommandBuilder()
    //     .setName('show')
